Shader "Mermaid/GoldScales2D"
{
    // The SampleScene golden-scales fluke shader, adapted for the 2D scene: rendered in the
    // TRANSPARENT queue with ZWrite Off so the MeshRenderer's sortingOrder interleaves the
    // 3D fluke tubes into the sprite sort (opaque depth writes would clip against / punch
    // through the z=0 sprites). Shading is identical to Mermaid/GoldScales.
    Properties
    {
